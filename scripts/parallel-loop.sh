...
merge_worker() {
  ...
  if [ "$wt_status" != "complete" ] && [ "$wt_status" != "ready-for-review" ]; then
    log "<<< Skipping merge for $task_name (status: $wt_status -- incomplete/timed out)"
    git worktree remove "$worktree" --force 2>/dev/null
    git branch -D "$branch" 2>/dev/null
  ...
  log "<<< Merging $task_name into dev (status: $wt_status)"

  local task_title
  task_title=$(bash "$REPO_ROOT/scripts/task-field.sh" "$REPO_ROOT/specs/tasks/$task_name.md" title 2>/dev/null || echo "$task_name")

  # Cherry-pick each worker commit onto dev.
  ...
  # Mark task complete on dev
  sed -i 's/^progress: .*/progress: complete/' "$REPO_ROOT/specs/tasks/$task_name.md"
  git add "$REPO_ROOT/specs/tasks/$task_name.md"
  git commit -m "chore: mark $task_name complete after merge" --no-verify 2>/dev/null || true

  # Push dev to origin so remote stays in sync
  git push origin dev 2>/dev/null || log "    !!! Push to origin failed -- will retry"

  emit merge.complete task="$task_name"
  log "    Merged $task_name ($task_title) into dev"

  git worktree remove "$worktree" --force 2>/dev/null
  git branch -D "$branch" 2>/dev/null
  unset "ACTIVE_WORKERS[$task_name]"
  log "    Cleaned up $task_name"
  return 0
}
