...
export class BrowsingTree {
  ...
  static #uint8ArrayToDataUrl(data) {
    let binary = '';
  ...
  async deactivateBranch(
    ...
    const storedNodes = descendants.map((node) => ({
      id: node.id,
      url: node.url,
      title: node.title,
  ...
  #decrementAncestorCounts(nodeId, count) {
    let current = this.nodes.get(nodeId);
    while (current) {
