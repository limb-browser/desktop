...
- **Commented-out code in patches:** Run `scripts/check-commented-code.sh`. Any added line in a patch file that contains commented-out code (e.g., `// functionCall(args);`, `// const x = y;`) is dead code and a finding. Commented-out wiring is especially dangerous — it circumvents dead-code checks (the symbols appear "used" inside the comment) and indicates the feature is non-functional despite appearing to be implemented. If the agent left a "Deferred" or "TODO" comment explaining why, the task does not meet its acceptance criteria — the feature must work, not be deferred.
