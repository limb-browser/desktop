...
<<<<<<< HEAD
- **Paired operation resource symmetry:** When a task implements inverse operations (activate/deactivate, save/restore), enumerate every resource side-effect of one direction and verify the inverse has a counterpart. If deactivation frees screenshots via `storage.deleteScreenshots()`, activation must restore them via `storage.loadScreenshots()` (or equivalent). Walk the spec steps for both directions — the spec often lists explicit steps for each (e.g., "Restore screenshots for loaded nodes"). A free without a restore is a data loss finding.
<<<<<<< HEAD
